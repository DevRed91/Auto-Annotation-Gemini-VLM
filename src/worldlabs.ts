...
export class WorldLabsService {
    ...
    async generateWorldFromImageUrl(imageUrl: string, textPrompt?: string): Promise<string> {
        return this.submitGeneration({
            type: 'image',
            image_prompt: {
                source: 'uri',
    ...
    private async submitGeneration(worldPrompt: WorldPrompt): Promise<string> {
        ...
        if (!response.ok) {
            throw new Error(`Generation failed: ${await this.readErrorBody(response)}`);
        }
    ...
    private async fetchWorldImageUrl(worldId: string): Promise<string> {
        ...
        if (!response.ok) {
            throw new Error(`World fetch failed: ${await this.readErrorBody(response)}`);
        }

        const data: any = await response.json();
        const imageUrl = data?.imagery?.pano_url
        ...
        return new Promise((resolve, reject) => {
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    if (!response.ok) {
                        clearInterval(interval);
                        reject(new Error(`Polling failed: ${await this.readErrorBody(response)}`));
                        return;
                    }
                    ...
                    if (data.done) {
                        ...
                        if (data.error) {
                            ...
                            const worldId = data.metadata?.world_id ?? data.response?.world_id;
                            const splatUrl = data.response?.assets?.splats?.spz_urls?.full_res
                            ...
                            if (worldId) {
                                ...
                                reject(new Error('Operation complete but no world ID or splat URL found'));
                            }
                        }
                    }
                } catch (error) {
